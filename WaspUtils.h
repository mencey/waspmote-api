...
class WaspUtils
{
  ...
  */
  ...
  /*!
  \param void
  \return free available memory expressed in Bytes
   */ 
  uint16_t getFreeMemory(void);
  ...
  \param void
  \return void
  \sa clearBuffer(),strExplode(const char* str, char separator)
   */
  ...
  \param void
  \return void
  \sa clearArguments(),strExplode(const char* str, char separator)
   */
  void clearBuffer(void) {for (int i = 0; i < GPS_BUFFER_SIZE; i++) inBuffer[i] = '\0';};
